...
import { Activity, BarChart3, BadgeRussianRuble, Boxes, Building2, ChevronDown, ChevronRight, CircleDollarSign, Gauge, LayoutDashboard, Menu, PackageSearch, PanelLeftClose, PanelLeftOpen, PieChart, Search, Settings2, ShoppingBag, Target, UserRound, Users, Warehouse, X, Zap } from "lucide-react";
...
const managementIcons: Record<(typeof managementTabs)[number]["href"], LucideIcon> = {
  "/management/organizations": Building2,
  "/management/cabinets": PackageSearch,
  "/management/users": UserRound,
  "/management/invitations": Users,
  "/management/team": Users,
  "/management/sync": Settings2,
};

const navigationSections: NavSection[] = [
  { items: [{ label: "Главная", href: "/dashboard", icon: LayoutDashboard }, { label: "Рука на пульсе", href: "/pulse", icon: Activity }, { label: "Сводный отчёт", href: "/reports/summary", icon: BarChart3 }] },
...
export function AppShell({ children, initialSession = {} }: { children: React.ReactNode; initialSession?: SessionViewModel }) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") { setOrganizationMenuOpen(false); setCabinetMenuOpen(false); setProfileMenuOpen(false); } };
    const onPointerDown = (event: MouseEvent) => { if (menuRef.current && !menuRef.current.contains(event.target as Node)) { setOrganizationMenuOpen(false); setCabinetMenuOpen(false); setProfileMenuOpen(false); } };
  ...
  const selectedOrganization = organizations.find((organization) => organization.id === searchParams.get("organization_ids"));
  const submitSearch = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const params = new URLSearchParams(searchParams.toString()); if (searchValue.trim()) params.set("search", searchValue.trim()); else params.delete("search"); window.location.assign(`/products?${params.toString()}`); };
  const logout = async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.assign("/signin"); };
  ...
  return <div className="min-h-screen bg-[#f4f5f3]"><aside className={`fixed inset-y-0 left-0 z-40 hidden bg-[#18362c] transition-[width] duration-200 lg:flex lg:flex-col ${collapsed ? "w-[68px]" : "w-[232px]"}`}>{sidebar}</aside>{mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 bg-black/35" onClick={() => setMobileOpen(false)} aria-label="Закрыть меню" /><aside className="relative flex h-full w-[280px] flex-col bg-[#18362c] shadow-xl">{sidebar}</aside></div>}<div className={`transition-[padding] duration-200 ${collapsed ? "lg:pl-[68px]" : "lg:pl-[232px]"}`}><header ref={menuRef} className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-[#dfe2dc] bg-white/95 px-4 backdrop-blur md:px-6"><button className="icon-button lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Открыть меню"><Menu size={20} /></button><form onSubmit={submitSearch} className="relative hidden max-w-sm flex-1 md:block"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#858b84]" size={16} /><input value={searchValue} onChange={(event) => setSearchValue(event.target.value)} className="h-9 w-full rounded-md border border-[#dfe2dc] bg-[#f7f8f6] pl-9 pr-3 text-xs outline-none focus:border-[#34745f]" placeholder="Найти товар, отчёт или кабинет" aria-label="Поиск" /></form><div className="ml-auto flex items-center gap-2"><div className="relative hidden sm:block"><button type="button" className="top-control" onClick={() => { setOrganizationMenuOpen((value) => !value); setCabinetMenuOpen(false); setProfileMenuOpen(false); }} aria-expanded={organizationMenuOpen} aria-haspopup="menu">{selectedOrganization?.name ?? session.organization_name ?? organizations[0]?.name ?? "Организация"}<ChevronDown size={14} /></button>{organizationMenuOpen && <div className="absolute right-0 top-11 z-50 w-64 rounded-md border border-[#dfe2dc] bg-white p-1.5 shadow-lg" role="menu"><p className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-[#929790]">Организация</p>{organizations.length ? organizations.map((organization) => <Link key={organization.id} href={filterHref("organization_ids", organization.id)} onClick={() => setOrganizationMenuOpen(false)} className="block rounded px-2.5 py-2 text-xs text-[#555b54] hover:bg-[#f4f5f3]" role="menuitem">{organization.name}</Link>) : <p className="px-2.5 py-2 text-xs text-[#929790]">Нет доступных организаций</p>}</div>}</div><div className="relative"><button type="button" className="top-control" onClick={() => { setCabinetMenuOpen((value) => !value); setOrganizationMenuOpen(false); setProfileMenuOpen(false); }} aria-expanded={cabinetMenuOpen} aria-haspopup="menu"><span className="size-2 rounded-full bg-[#34745f]" />{selectedCabinet?.name ?? "Все кабинеты"}<ChevronDown size={14} /></button>{cabinetMenuOpen && <div className="absolute right-0 top-11 z-50 w-64 rounded-md border border-[#dfe2dc] bg-white p-1.5 shadow-lg" role="menu"><p className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-[#929790]">Кабинет</p><Link href={filterHref("cabinet_ids")} onClick={() => setCabinetMenuOpen(false)} className="block rounded px-2.5 py-2 text-xs text-[#555b54] hover:bg-[#f4f5f3]" role="menuitem">Все кабинеты</Link>{cabinets.length ? cabinets.map((cabinet) => <Link key={cabinet.id} href={filterHref("cabinet_ids", cabinet.id)} onClick={() => setCabinetMenuOpen(false)} className="block rounded px-2.5 py-2 text-xs text-[#555b54] hover:bg-[#f4f5f3]" role="menuitem">{cabinet.name}<span className="ml-1 text-[10px] text-[#929790]">· {cabinet.marketplace}</span></Link>) : <p className="px-2.5 py-2 text-xs text-[#929790]">Нет подключённых кабинетов</p>}</div>}</div><div className="relative"><button type="button" className="icon-button" onClick={() => { setProfileMenuOpen((value) => !value); setOrganizationMenuOpen(false); setCabinetMenuOpen(false); }} aria-label="Меню профиля" aria-expanded={profileMenuOpen} aria-haspopup="menu"><span className="grid size-7 place-items-center rounded-full bg-[#eaf5ef] text-[10px] font-semibold text-[#34745f]">{session.user?.username?.slice(0, 2).toUpperCase() ?? "—"}</span></button>{profileMenuOpen && <div className="absolute right-0 top-11 z-50 w-64 rounded-md border border-[#dfe2dc] bg-white p-2 shadow-lg" role="menu"><p className="px-2.5 py-1 text-xs font-semibold text-[#2a2e29]">{session.user?.username ?? "Пользователь"}</p><p className="px-2.5 pb-2 text-[10px] text-[#858b84]">{session.user?.email ?? ""} · {session.role ?? "роль не определена"}</p><Link href="/management/team" onClick={() => setProfileMenuOpen(false)} className="block rounded px-2.5 py-2 text-xs text-[#555b54] hover:bg-[#f4f5f3]" role="menuitem">Команда</Link><button className="mt-1 flex w-full rounded px-2.5 py-2 text-left text-xs text-[#a54b31] hover:bg-[#fff3f0]" onClick={() => void logout()} role="menuitem">Выйти</button></div>}</div></div></header><main className="min-h-[calc(100vh-64px)] p-4 md:p-6 xl:p-7">{children}</main></div></div>;
}
