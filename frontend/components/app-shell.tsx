...
const sections: NavSection[] = [
  ...
  { label: "Управление", items: [{ label: "Кабинеты", href: "/management/cabinets", icon: PackageSearch, permission: "cabinet:manage" }, { label: "Команда", href: "/management/team", icon: Users, permission: "team:manage" }, { label: "Синхронизация", href: "/management/sync", icon: Settings2 }] },
];

export function AppShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const searchParams = useSearchParams();
  const [collapsed, setCollapsed] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const [session, setSession] = useState<SessionViewModel>({});
  const suffix = searchParams.toString() ? `?${searchParams.toString()}` : "";
  useEffect(() => {
    let active = true;
    Promise.all([apiRequest<UserResponse>("/api/v1/users/me"), apiRequest<PageResult<OrganizationResponse>>("/api/v1/organizations?page=1&page_size=100")]).then(async ([user, organizations]) => {
  ...
  const visibleSections = useMemo(() => sections.map((section) => ({ ...section, items: section.items.filter((item) => !item.permission || can(role, item.permission)) })).filter((section) => section.items.length), [role]);
