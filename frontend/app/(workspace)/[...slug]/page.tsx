...
export default async function WorkspacePage({ params, searchParams }: Props) {
  ...
  if (path === "/dashboard") { const result = await getOverview(query); if (result.error) return <DataError message={result.error.message} requestId={result.error.requestId} status={result.error.status} />; return <DashboardView data={result.data!} />; }
  if (slug[0] === "products" && slug.length === 2 && !["unit-economics", "stocks"].includes(slug[1])) { const result = await getProductDetail(slug[1]); if (result.error) return <DataError message={result.error.message} requestId={result.error.requestId} status={result.error.status} />; return <ProductDetail product={result.data!} />; }
  const result = await getOperationalPage(path, query);
  if ("notFound" in result) return notFound();
  if (result.error) return <DataError message={result.error.message} requestId={result.error.requestId} status={result.error.status} />;
